...
- The workspace now includes every package it lists, the root and packages without a `name` or `version` or with a duplicated name included. Such packages are never versioned, `get-packages --all` lists them without the missing fields, and naming one in a changeset or `--ignore` is an error.
