...
- Every internal dependency range on a released package, in released and unreleased packages alike, is raised to the new version: `^1.0.0` becomes `^1.0.1`, `>=1.0.0 <2.0.0` becomes `>=1.0.1 <2.0.0`, and a range the new version leaves keeps its shape, as in `^2.0.0`. `workspace:*`, `workspace:^`, and `workspace:~` are left as they are, and a snapshot release pins the snapshot version.
